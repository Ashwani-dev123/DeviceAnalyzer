...
class StorageProvider(
    ...
    fun calculateTotalInternalStorage(): String {
        val totalInternalStorageBytes = Environment.getDataDirectory().totalSpace
        val totalInternalStorageGB = totalInternalStorageBytes.toDouble() / (1024.0 * 1024.0 * 1024.0)
        return String.format("%.1f", totalInternalStorageGB)
    }

    fun calculateFreeInternalStorage(): String {
        val freeInternalStorageBytes = Environment.getDataDirectory().freeSpace
        val freeInternalStorageGB = freeInternalStorageBytes.toDouble() / (1024.0 * 1024.0 * 1024.0)
        return String.format("%.1f", freeInternalStorageGB)
    }

    fun calculateTotalUsedInternalStorage(): String {
        val totalInternalStorageBytes = Environment.getDataDirectory().totalSpace
        val freeInternalStorageBytes = Environment.getDataDirectory().freeSpace
        val usedInternalStorageBytes = totalInternalStorageBytes - freeInternalStorageBytes
        val usedInternalStorageGB = usedInternalStorageBytes.toDouble() / (1024.0 * 1024.0 * 1024.0)
        return String.format("%.1f", usedInternalStorageGB)
    }
